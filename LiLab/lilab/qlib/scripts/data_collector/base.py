...
class BaseNormalize(abc.ABC):
    def __init__(self, date_field_name: str = "date", symbol_field_name: str = "symbol",                  
                index_filter=None,
                index_start_time=None,
                index_end_time=None,
                **kwargs):
...
class Normalize:
    ...
    def _executor(self, file_path: Path):
        ...
        # df[self._date_field_name] = df[self._date_field_name].apply(lambda s: s.split('+')[0])
    ...
    def normalize(self, act_list=None):
        logger.info("normalize data......")

        with ProcessPoolExecutor(max_workers=self._max_workers) as worker:
            _file_list = list(self._source_dir.glob("*.csv"))
            # if act_list is not None:
            #     # print(act_list)
            #     file_list = []
            #     for file in _file_list:
            #         # print(os.path.basename(file))
            #         if os.path.basename(file).split('.')[0][2:] in act_list:
            #             file_list.append(file)
            #     logger.info(f'Aftet filter: {len(file_list)}')
            # else:
