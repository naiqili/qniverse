...
class YahooCollector(BaseCollector):
    ...
    def get_data_from_remote(symbol, interval, start, end, show_1min_logging: bool = False):
        ...
        try:
            ...
            _resp = Ticker(symbol, asynchronous=False,  proxies=proxies).history(interval=interval, start=start, end=end)
...
def index_filter_func(symbols, index_filter, index_start_time, index_end_time):
    act_list = []
    if index_filter == 'csi100':
        instruments = D.instruments(market='csi100')
        act_list = D.list_instruments(instruments=instruments, start_time=index_start_time, \
                                        end_time=index_end_time, as_list=True)
        act_list = [s[2:] for s in act_list]
        symbols = [s for s in symbols if s[:6] in act_list]
    if index_filter == 'csi100_main':
        nameDFilter = NameDFilter(name_rule_re='(SH60[0-9]{4})|(SZ00[0-9]{4})|SH000300')
        instruments = D.instruments(market='csi100', filter_pipe=[nameDFilter])
        act_list = D.list_instruments(instruments=instruments, start_time=index_start_time, \
                                        end_time=index_end_time, as_list=True)
        act_list = [s[2:] for s in act_list]
        symbols = [s for s in symbols if s[:6] in act_list]
    if index_filter == 'csi300':
        instruments = D.instruments(market='csi300')
        act_list = D.list_instruments(instruments=instruments, start_time=index_start_time, \
                                        end_time=index_end_time, as_list=True)
        act_list = [s[2:] for s in act_list]
        symbols = [s for s in symbols if s[:6] in act_list]
    if index_filter == 'csi300_main':
        nameDFilter = NameDFilter(name_rule_re='(SH60[0-9]{4})|(SZ00[0-9]{4})|SH000300')
        ...
        act_list = [s[2:] for s in act_list]
        symbols = [s for s in symbols if s[:6] in act_list]
    if index_filter == 'csi500':
        instruments = D.instruments(market='csi500')
        act_list = D.list_instruments(instruments=instruments, start_time=index_start_time, \
                                        end_time=index_end_time, as_list=True)
        act_list = [s[2:] for s in act_list]
        symbols = [s for s in symbols if s[:6] in act_list]
    if index_filter == 'csi500_main':
        nameDFilter = NameDFilter(name_rule_re='(SH60[0-9]{4})|(SZ00[0-9]{4})|SH000300')
...
class YahooCollectorCN(YahooCollector, ABC):
    def get_instrument_list(self):
        ...
        symbols, _ = index_filter_func(symbols, self.index_filter, self.index_start_time, self.index_end_time)
...
class YahooNormalize(BaseNormalize):
    ...
    def normalize_yahoo(
        ...
        df = df.copy()
        # df[date_field_name] = df[date_field_name].apply(lambda s: s.split('+')[0] if '+' in s \
        #                                                 else s+" 09:30:00")
        df[date_field_name] = df[date_field_name].apply(lambda s: s.split()[0])
...
class Run(BaseRun):
    ...
    def download_data(
        ...
        super(Run, self).download_data(max_collector_count, delay, start, end, check_data_length, \
                                       limit_nums, index_filter, index_start_time,index_end_time)
    ...
    def normalize_data_1d_extend(
        ...
        #     _, act_list = index_filter_func([], index_filter, index_start_time, index_end_time)
        yc.normalize()
    ...
    def update_data_to_bin(
        ...
        self.download_data(delay=delay, start=trading_date, end=end_date, \
                           index_start_time=index_start_time,
                           index_end_time=index_end_time,
                           check_data_length=check_data_length, index_filter=index_filter)
