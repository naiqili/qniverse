...
class TopkDropoutStrategy(BaseSignalStrategy):
    ...
    def generate_trade_decision(self, execute_result=None, pred_score=None, today=None):
        ...
        if pred_score is None:
            ...
            trade_start_time, trade_end_time = \
                pd.to_datetime(today), pd.to_datetime(today)
        ...
        for code in current_stock_list:
            if pred_score is None and not self.trade_exchange.is_stock_tradable(
                stock_id=code,
...
class TopkDropoutBadStrategy(BaseSignalStrategy):
    # TODO:
    # 1. Supporting leverage the get_range_limit result from the decision
    # 2. Supporting alter_outer_trade_decision
    # 3. Supporting checking the availability of trade decision
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        # last position (sorted by score)
        # last = pred_score.reindex(current_stock_list).sort_values(ascending=False).index
        sell_df = pred_score.reindex(current_stock_list).sort_values(ascending=False)
        # print(sell_df)
        sell_df = sell_df.loc[sell_df < self.bad_thresh]
        last = sell_df.index
        self.n_drop = len(sell_df)
        # print(self.n_drop)
        # The new stocks today want to buy **at most**
        if self.method_buy == "top":
            today = get_first_n(
